...
class Model(nn.Module):
    def __init__(self, inc, outc, corr=True, pretrained=True):
        ...
        # attention
        self.sa = PAM_Module(1024)
        self.sc = CAM_Module(1024)
    ...
    def forward(self, x):
        ...
        # attention model
        enc1_sa_feature = self.sa(enc1_f6)
        enc1_sc_feature = self.sc(enc1_f6)
        enc1_sac_feature = enc1_sa_feature + enc1_sc_feature

        enc2_sa_feature = self.sa(enc2_f6)
        enc2_sc_feature = self.sc(enc2_f6)
        enc2_sac_feature = enc2_sa_feature + enc2_sc_feature


        # decoder
        #enc_f6 = torch.cat([enc1_f6, enc2_f6], 1)
        enc_f6 = torch.cat([enc1_sac_feature, enc2_sac_feature], 1)
