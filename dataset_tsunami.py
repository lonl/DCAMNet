...
class TSUNAMI(Dataset):
    ...
    def __getitem__(self, index):
        ...
        fn_mask = get_img_path(self.mask_root, filename, '.bmp')
...
class TSUNAMI_full(Dataset):

    def __init__(self, root, id_s, id_e, width, height):
        super(TSUNAMI_full, self).__init__()
        self.img_t0_root = os.path.join(root, 't0')
        self.img_t1_root = os.path.join(root, 't1')
        self.mask_root = os.path.join(root, 'ground_truth')
