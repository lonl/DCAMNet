...
class Test:
    ...
    def test(self):
        ...
        print("self.h_resize, self.w_resize: ", self.h_resize, self.w_resize)
    ...
    def display_results(self, t0, t1, mask_pred, mask_gt):
        ...
        img_out[0:h, 0:w, :] = t0_disp[...,::-1]
        img_out[0:h, w:w * 2, :] = t1_disp[...,::-1]
        img_out[h:h * 2, 0:w * 1, :] = mask_gt_disp[...,::-1]
    ...
    def run(self):
        ...
        for i_set in range(0,self.di.num_cv):
            if self.args.use_corr:
                ...
                fn_model = os.path.join(os.path.join(self.args.checkpointdir, 'set{}'.format(i_set), 'cscdnet-00017000.pth'))
            else:
                print('Siamese Change Detection Network (Siamese CDResNet)')
                self.model = cscdnet.Model(inc=6, outc=2, corr=False, pretrained=True)
                fn_model = os.path.join(os.path.join(self.args.checkpointdir, 'set{}'.format(i_set), 'cdnet-00001000.pth'))
            ...
            if self.args.dataset == 'PCD':
                from dataset_tsunami import TSUNAMI_full
                #from dataset_pcd import PCD_full
                #for dataset in ['TSUNAMI', 'GSV']:
                for dataset in ['TSUNAMI']:
                    loader_test = TSUNAMI_full(os.path.join(self.args.datadir, 'set0', 'test'), self.di.no_start, self.di.no_end, self.di.width, self.di.height)
                    ...
                    for index in range(0,loader_test.__len__()):
                        if i_set * (10 / self.di.num_cv) <= (index % 10) < (i_set + 1) * (10 / self.di.num_cv):
                            ...
                            continue
            elif self.args.dataset == 'cdnet':
                from datasets import CD2014

                cdnet_path = "/mnt/a409/users/hanpengcheng/DataSets/CDnet/cd2014_new_branch/cd20142"
                ...
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/baseline/baseline_8.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/cameraJitter/badminton_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/shadow/shadow_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/badWeather/badWeather_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/intermittentObjectMotion/intermittentObjectMotion_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/nightVideos/nightVideos_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/thermal/thermal_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/dynamicBackground/dynamicBackground_1.txt"
                #cdnet_TRAIN_TXT_PATH = cdnet_path + "/AICD/AICD_1.txt"
                cdnet_TRAIN_TXT_PATH = cdnet_path + "/baseline/baseline_1.txt"
                print("cdnet_path: ", cdnet_path)
                print("cdnet_TRAIN_TXT_PATH: ", cdnet_TRAIN_TXT_PATH)

                loader_test = CD2014.Dataset_val(cdnet_TRAIN_DATA_PATH, cdnet_TRAIN_LABEL_PATH,
                               cdnet_TRAIN_TXT_PATH, 'val', transform=True, transform_med=train_transform_det)

                for index in range(0, loader_test.__len__()):
                    if True:
                        self.index = index
                        self.t0, self.t1, self.mask, self.w_orig, self.h_orig, self.w_resize, self.h_resize = loader_test.__getitem__(
                            index)
                        self.test()
                    else:
                        continue
