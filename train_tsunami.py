...
from dataset_tsunami import TSUNAMI
...
class Training:
    ...
    def train(self):
        ...
        if dataset_type == "AICD":

            pass

        if dataset_type == "PCD":
            dataset_train = DataLoader(
                TSUNAMI(os.path.join(self.args.datadir, 'set{}'.format(self.args.cvset), 'train')),
                num_workers=self.args.num_workers, batch_size=self.args.batch_size, shuffle=True)
            self.dataset_test = TSUNAMI(os.path.join(self.args.datadir, 'set{}'.format(self.args.cvset), 'test'))
        else:
        ...
        while self.icount < self.args.max_iteration:
            ...
            for step, (inputs_train, mask_train) in enumerate(dataset_train):
                ...
                if self.args.icount_plot > 0 and self.icount % self.args.icount_plot == 0 and dataset_type == "PCD":
                    self.test()
